"""
Basically the exact same as the original generate data but adapted with temperature and other parameters
that are fed into the model generation to allow for the eigenvalue calculation pipeline

Will use the penultimate layer as stated in the INSIDE paper
"""
...
argument_parser.add_argument("--output_path", type=str, default="./MIND/auto-labeled/eigen_output")
...
output_path = args.output_path + f"/{model_family}_{model_type}_{step_num}000"
...
import torch
from utils.model_eigen import get_model
from utils.gen import chat_change_with_answer
from tqdm import tqdm
import json
import spacy

model, tokenizer, generation_config, at_id = get_model(model_type, model_family, model_checkpoint=model_checkpoint)
...
for data_type in ["train", "valid", "test"]:
    ...
    for ii, d in tqdm(enumerate(data)):
        ...
        for e, idx in entities:
            ...
            print(f'{prompt_chat}\t{sequences.shape}')
