from transformers import GPTNeoXForCausalLM, LlamaForCausalLM, LlamaTokenizer, AutoModelForCausalLM, AutoTokenizer, AutoConfig
import torch

def get_model(model_type, model_family, max_new_tokens=1, model_checkpoint=None):
    if model_family == "opt":
        if model_type == "7b":
    ...
    if model_family == "falcon" or model_family == "gptj":
        ...
        # Editting the configs according to the INSIDE framework
        generation_config = dict(
                            top_k=5,
                            top_p=0.99,
                            temperature=0.5,
                            ...
                            num_return_sequences=2,   # 10 generations as stated in the paper
            )
    else:
        generation_config = dict(
                            top_k=0,
