...
def main():
    ...
    wandb.init(
        project=f"llama8B-NORMAL-{model_version}-{model_type}-{data_path.split('/')[-1].split('.')[0]}",
        config={
    ...
    model_name = f"llama8b_normal_{model_type}-{data_path.split('/')[-1].split('.')[0]}_{iteration}"
    model.save_pretrained(f"{SCRATCH_DIR}/llama_8b_normal/{model_type}-{data_path.split('/')[-1].split('.')[0]}")
