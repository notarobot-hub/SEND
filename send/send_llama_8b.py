...
def main():
    ...
    argument_parser.add_argument("--model_version", type=str, default="3.2")
    ...
    MODEL_NAME = f"meta-llama/Llama-{model_version}-{model_type}-Instruct"
    ...
    wandb.init(
        project=f"final_llama8B-SEND-{model_version}-{model_type}-{data_path.split('/')[-1].split('.')[0]}",
        config={
    ...
    print("Freexing first 24/32 layers")
    for i in range(13):
    ...
    wandb.finish()
    # save the end model to SCRATCH_DIR/llama_8b_send dir
    os.makedirs(f"{SCRATCH_DIR}/llama_8b_send", exist_ok=True)
    model_name = f"llama8b_SEND_{model_type}-{data_path.split('/')[-1].split('.')[0]}-{iteration}"
    model.save_pretrained(f"{SCRATCH_DIR}/llama_8b_send/{model_type}-{data_path.split('/')[-1].split('.')[0]}")
    model.push_to_hub(f"Shahradmz/{model_name}")
